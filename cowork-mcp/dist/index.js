...
function getClient() {
    ...
    if (!apiKey) {
        throw new Error('ANTHROPIC_API_KEY is not set. Add it to .env.secrets or environment.');
    }
...
function handleError(error) {
    if (error instanceof Anthropic.APIError) {
        if (error.status === 401)
            return 'Error: Invalid ANTHROPIC_API_KEY. Check your credentials.';
        if (error.status === 429)
            return 'Error: Rate limit exceeded. Please wait before retrying.';
        if (error.status === 529)
            return 'Error: Claude API is overloaded. Please retry shortly.';
...
async function callClaude(messages, system, model = DEFAULT_MODEL) {
...
Examples:
  ...
  - "Explain the difference between TCP and UDP"`,
        ...
        try {
            const reply = await callClaude([{ role: 'user', content: task }], system, model ?? DEFAULT_MODEL);
    ...
    server.registerTool('claude_multimodal_task', {
  ...
  - Compare two diagrams`,
        ...
        try {
            ...
            if (system)
                params.system = system;
            const response = await client.messages.create(params);
            const reply = truncate(response.content
                .filter((b) => b.type === 'text')
                .map((b) => b.text)
                .join('\n'));
    ...
    server.registerTool('claude_code_review', {
...
Be concise and actionable. Focus: ${focus}.`;
    ...
    server.registerTool('claude_summarize', {
...
Returns:
  The summary in the requested format.`,
        ...
        try {
            ...
            const formatInstruction = format === 'bullets' ? 'Use a concise bulleted list.' :
            ...
            const reply = await callClaude([{ role: 'user', content: `Summarize the following:\n\n${content}` }], system);
            return { content: [{ type: 'text', text: reply }] };
        }
        catch (err) {
            return { content: [{ type: 'text', text: handleError(err) }] };
    ...
    server.registerTool('claude_brainstorm', {
...
Returns:
  A numbered list of ideas or steps with brief explanations.`,
        ...
        try {
            ...
            const typeInstruction = output_type === 'steps' ? `Generate ${count} concrete, actionable steps to achieve the goal.` :

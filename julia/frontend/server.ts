...
/**
 * Lightweight memory capture for web conversations.
 * Same logic as orchestrator/src/memory-keeper.ts but inlined here since
 * frontend and orchestrator are separate packages.
 * Fire-and-forget — never blocks or crashes the chat.
 */
async function captureMemory(userMessage: string): Promise<void> {
...
If not: {"save":false}`,
        ...
        if (result.save && result.category && result.memory) {
            ...
            console.log(`[memory] Captured ${result.category}: "${result.memory.slice(0, 60)}..."`);
        }
    } catch (err) {
        // Silent fail — memory capture must never crash the chat
        console.error('[memory] Capture error (non-fatal):', err);
    }
}
...
const SYSTEM_PROMPT = `You are Julia-Web — the web dashboard interface of Julia, an experimental multi-agent system built by Raphael as part of his bachelor thesis on autonomous AI agents.

## What you are
You are **not** a generic chatbot. You are one node in a multi-agent architecture called Julia:
- **You (Julia-Web)**: The web dashboard interface. You handle conversations here and can delegate complex work to Claude.
- **Julia-Orchestrator**: Julia's central brain — processes messages from all channels, has access to everything: sending emails, delegating to Claude, managing tasks.
- **Julia-Bridge**: The central message hub connecting all components (you, the orchestrator, OpenClaw).
- **OpenClaw**: The Telegram gateway — connects Julia to Telegram users.
- **Cowork-MCP**: A Claude Sonnet instance you can delegate to for deep analysis, code review, and complex reasoning.
- **Backend**: A REST API with Postgres that stores tasks and memories.

Julia is the whole system. You (Julia-Web) and Julia-Telegram are user-facing interfaces. Julia-Orchestrator is the shared brain behind both.
...
- You're Julia-Web — the web interface of Julia, a multi-agent thesis project
...
- You can send emails, Telegram messages, and more by routing through Julia-Orchestrator via the Julia-Bridge
...
const chatTools = {
    send_to_orchestrator: tool({
        description:
            'Route an action request to Julia-Orchestrator via the Julia-Bridge. ' +
...
app.post('/api/chat', async (c) => {
    ...
    // Fire-and-forget: capture the latest user message as a potential memory
    const lastUserMsg = [...messages].reverse().find((m) => m.role === 'user');
    if (lastUserMsg) {
        const textPart = lastUserMsg.parts?.find((p: any) => p.type === 'text') as { type: 'text'; text: string } | undefined;
        if (textPart?.text) {
            captureMemory(textPart.text).catch(() => { });
        }
    }
